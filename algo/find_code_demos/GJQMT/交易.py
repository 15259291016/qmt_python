...
class timeDecided:
    ...
    def addObj(self, priceObj):
        ...
        if len(self.Fmin_price_list) < 115:
            ...
            self.Fmin_price_list.append(priceObj)
        print((priceObj[accountObj['code_list'][0]]["amount"] - accountObj["amount"]) / 100)
        if (priceObj[accountObj['code_list'][0]]["amount"] - accountObj["amount"]) / 100 > 10000:
            self.min_big_price_list.append(5)
            self.Fmin_big_price_list.append(5)
            self.all_big_price_list.append(5)
        elif (priceObj[accountObj['code_list'][0]]["amount"] - accountObj["amount"]) / 100 > 6000:
            self.min_big_price_list.append(2)
            self.Fmin_big_price_list.append(2)
            self.all_big_price_list.append(2)
        elif (priceObj[accountObj['code_list'][0]]["amount"] - accountObj["amount"]) / 100 > 3000:
            self.min_big_price_list.append(1)
            self.Fmin_big_price_list.append(1)
            self.all_big_price_list.append(1)
        else:
    ...
    def predict(self):
        if sum(self.min_big_price_list) > 6 and sum(self.min_big_price_list[int(len(self.min_big_price_list) / 2):]) > 10:
            if sum(self.Fmin_big_price_list) > 15:
                return True
        return False
...
def handlebar(C):
    ...
    if accountObj['amount'] != 0:
        ...
        print(f'成本价:{accountObj["positions"]["m_dOpenPrice"]:.2f}----个股成交量：{(tick[code]["amount"] - accountObj["amount"]) / 100}手 ---- 个股成交额：{(tick[code]["amount"] - accountObj["amount"]) * tick[code]["bidPrice"][0] / 10000:.4f} 万  换手率{((tick[code]["amount"] - accountObj["amount"]) / 100) / tick[code]["amount"] * 100:.4f}%')
        print(decider1.predict())
        if (tick[code]["amount"] - accountObj["amount"]) / 100 > 10000:
            if accountObj["bidVol"] > sum(tick[code]["bidVol"]) * 2:
